...
export class App {
  ...
  protected matchesMonth(dateStr: string, targetMonth: string): boolean {
  ...
  protected isSavingsContribution(item: Transaction): boolean {
  ...
  protected isSavingsWithdrawal(item: Transaction): boolean {
    if (!item.savings) return false;
    const fundType = String(item.fundType || '').toLowerCase();
  ...
  protected readonly monthlySavingsWithdrawals = computed(() =>
    this.savingsTransactions()
      .filter((item) => this.isSavingsWithdrawal(item) && this.matchesMonth(item.date, this.selectedMonth()) && !this.isFailedTransaction(item))
      .reduce((sum, item) => sum + item.amount, 0)
  );
  ...
  protected isFailedTransaction(item: Transaction | null | undefined): boolean {
    ...
    const isFlagged = Boolean((item as any).flagged || (item as any).failed);
    return isFlagged ||
  ...
  protected readonly savingsSummary = computed<SavingsMonthlySummary[]>(() => {
    ...
    return months.map((month) => {
      const monthTransactions = this.savingsTransactions().filter((item) => this.matchesMonth(item.date, month) && !this.isFailedTransaction(item));
  ...
  protected readonly currentSavingsSummary = computed(() => {
  ...
  protected readonly monthlyContribution = computed(() => this.currentSavingsSummary().contributions);
  protected readonly effectiveMonthlySavings = computed(() => {
  ...
  protected categoryTotal(category: string): number {
    return this.selectedTransactions()
      .filter((item) => !item.savings && (item.type === 'Expense' || !item.type) && (item.category || '').trim().toLowerCase() === (category || '').trim().toLowerCase())
      .reduce((sum, item) => sum + Math.abs(Number(item.amount) || 0), 0);
  }
